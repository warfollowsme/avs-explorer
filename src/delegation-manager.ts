...
export function handleWithdrawalMigrated(event: WithdrawalMigratedEvent): void {
  ...
  let oldWithdrawalRoot = WithdrawalRoot.load(event.params.oldWithdrawalRoot)
  if(oldWithdrawalRoot != null) {  
    let withdrawal = Withdrawal.load(oldWithdrawalRoot.withdrawal)
    if (withdrawal != null) {
      let newWithdrawalRoot = new WithdrawalRoot(event.params.newWithdrawalRoot)
      newWithdrawalRoot.withdrawal = withdrawal.id
      newWithdrawalRoot.save()

      withdrawal.oldRoot = event.params.oldWithdrawalRoot
      withdrawal.root = event.params.newWithdrawalRoot
...
export function handleWithdrawalQueued(event: WithdrawalQueuedEvent): void {
  ...
  let withdrawalId = event.params.withdrawal.staker.concatI32(event.params.withdrawal.nonce.toI32())
  ...
  if(withdrawal == null){
    ...
    withdrawal.nonce = event.params.withdrawal.nonce
    ...
    for (let i = 0; i < event.params.withdrawal.strategies.length; i++) {
      ...
      let withdrawalStrategyId = withdrawalId.concat(s)
      ...
      withdrawalStrategy.withdrawal = withdrawalId
    ...
    action.transactionHash = event.transaction.hash
    //params
    action.withdrawal = withdrawalId
